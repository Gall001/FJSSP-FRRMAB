...
class GA:
    ...
    def __init__(self, dataset, solutionNumber):
            ...
            for key, line in enumerate(data):
                ...
                while i < len(parsed_line):
                    ...
                    for id_operation in range(1, number_operations + 1):
                        ...
                        operation_tuple = int(
                            parsed_line[i + 2 * id_operation - 1]), int(parsed_line[i + 2 * id_operation])
        ...
        for j in range(len(self.jobs_list)):
            print("JOB [", j+1, "] has ",
                  len(self.jobs_list[j]), " operations")
            st.write("Job ", j+1, " has ",
                     len(self.jobs_list[j]), " operations")
        ...
        for i in range(self.SolutionNum):
            ...
            for x in range(len(self.jobs_list)):
                for y in self.jobs_list[x]:
                    ...
                    OperationChosen = self.jobs_list[x][y][random.randint(
                        0, len(self.jobs_list[x][y])-1)]
                    Grade = 1
                    for key in range(len(self.jobs_list[x][y])):
                        if OperationChosen[1] > self.jobs_list[x][y][int(key)-1][1]:
                            Grade = Grade + 1
                    OperationOrder += 1
        ...
        for x in range(len(self.jobs_list)):
            for y in self.jobs_list[x]:
                ...
                timeMachineOrganizedTemp = sorted(
                    timeMachineScrambled, key=lambda x: x[1])
        ...
        for i in range(len(self.SolutionList)):
            ...
            for x in range(len(self.SolutionList[i][0])):
                timeMachineTemp.append(
                    timeMachineOrganized[x][self.SolutionList[i][0][x][0] - 1])
    ...
    def methods(self, crossoverChance, mutationChance, action):
        # crossover
        for x in range(self.SolutionNum):
            CrossoverList = []
            s1 = self.SolutionList[random.randint(
                0, int(float(self.SolutionNum))-1)][0]
            s2 = self.SolutionList[random.randint(
                0, int(float(self.SolutionNum))-1)][0]
            ...
            for i in range(len(CrossoverList)):
                ...
                for job in range(len(self.jobs_list)):
                    ...
                    CrossoverList[i] = correction(
                        minValue, maxValue, CrossoverList[i])
    ...
    def showResult(self):
        st.write('Best solution sequence: ', min(
            self.solutionMachineTimeI, key=lambda x: x[-1]))
        st.write('Best solution found: ', min(
            self.solutionMachineTimeI, key=lambda x: x[-1])[-1])
